...
class h_DQN:
	def __init__(self, params, meta=False):
		"""
		...
		self.x = tf.placeholder('float', [None, params['input_dimension'][0], params['input_dimension'][1],
		...
		# conv 1
		layer_name = 'conv1' ;  filter_size = 8 ; channels = self.channels_num ; filters = 32 ; stride = 4
		...
		self.ip4 = tf.add(tf.matmul(self.o3_flat, self.w4), self.b4, name=self.network_name+'_'+layer_name+'_outputs')
		self.o4 = tf.nn.relu(self.ip4, name=self.network_name+'_'+layer_name+'_activations')
		
		# fully connected layer 2
		layer_name = 'fc2' ; hiddens = params['action_num'] ; dim = 256
